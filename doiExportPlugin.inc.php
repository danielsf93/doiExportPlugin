...
/**
 * @file plugins/importexport/native/doiExportPlugin.inc.php
 *
...
class doiExportPlugin extends ImportExportPlugin {
	...
	function register($category, $path, $mainContextId = null) {
		...
		if ($success && $this->getEnabled()) {
			$this->addLocaleData();
			$this->import('doiExportDeployment');
		}
		return $success;
	}
	...
	function display($args, $request) {
		...
		switch (array_shift($args)) {
			...
			case 'import':
				...
				$deployment = new doiExportDeployment($context, $user);
	...
	function exportSubmissions($submissionIds, $context, $user, $opts = array()) {
		...
		$exportFilter->setDeployment(new doiExportDeployment($context, $user));
		$submissions = array();
	...
	function executeCLI($scriptName, &$args) {
		...
		switch ($command) {
			case 'import':
				...
				$deployment = new doiExportDeployment($press, $user);
